...
class DataManager:
    ...
    def __init__(
        ...
        Args:
            ...
            domain_ratios: 领域采样比例，如 {"math": 0.4, "code": 0.3, "qa": 0.3}
                          或法律模式: {"legal_cn": 0.5, "legal_us": 0.5}
            shuffle: 是否打乱数据
        """
        ...
        # 检测是否为法律模式
        self.legal_mode = any("legal" in k for k in self.domain_ratios.keys())

        # 加载的数据
        if self.legal_mode:
            self.train_data = {"legal_cn": [], "legal_us": []}
            self.val_data = {"legal_cn": [], "legal_us": []}
            self.test_data = {"legal_cn": [], "legal_us": []}
            self.current_indices = {"legal_cn": 0, "legal_us": 0}
        else:
    ...
    def load_data(self, split: str = "train") -> Dict[str, List[Dict]]:
        ...
        # 法律模式：加载中美法律数据
        if self.legal_mode:
            return self._load_legal_data(split)
    ...
    def _load_legal_data(self, split: str = "train") -> Dict[str, List[Dict]]:
        """加载法律数据（中国和美国）"""
        data_by_type = defaultdict(list)
        ...
        # 加载中国法律数据
        cn_files = [
            legal_dir / "cn" / "training" / f"{split}.jsonl",
            legal_dir / "cn" / "training" / "disc_law_10k.json",
            legal_dir / "cail2018" / f"{split}.jsonl"
        ]
                    ...
                    with open(file_path, 'r', encoding='utf-8') as f:
                        if file_path.suffix == '.jsonl':
                            for line in f:
                                if line.strip():
                                    sample = json.loads(line)
                                    sample['jurisdiction'] = 'CN'
                                    sample['problem_type'] = 'legal'
                                    data_by_type['legal_cn'].append(sample)
                                    source_stats[sample.get('source', 'cn_unknown')] += 1
                        else:  # .json
                            data = json.load(f)
                            if isinstance(data, list):
                                for sample in data:
                                    ...
                                    data_by_type['legal_cn'].append(sample)
                                    source_stats[sample.get('source', 'cn_unknown')] += 1
                    print(f"✅ 加载CN法律数据: {file_path.name}")
                except Exception as e:
                    print(f"⚠️  加载失败 {file_path}: {e}")

        # 加载美国法律数据
        us_files = [
            legal_dir / "us" / "training" / f"{split}.jsonl",
            legal_dir / "us" / "legalbench" / "legalbench_test.json",
            legal_dir / "us" / "casehold" / f"{split}.jsonl"
        ]
                    ...
                    with open(file_path, 'r', encoding='utf-8') as f:
                        if file_path.suffix == '.jsonl':
                            for line in f:
                                if line.strip():
                                    sample = json.loads(line)
                                    sample['jurisdiction'] = 'US'
                                    sample['problem_type'] = 'legal'
                                    data_by_type['legal_us'].append(sample)
                                    source_stats[sample.get('source', 'us_unknown')] += 1
                        else:
                            data = json.load(f)
                            if isinstance(data, list):
                                for sample in data:
                                    ...
                                    data_by_type['legal_us'].append(sample)
                                    source_stats[sample.get('source', 'us_unknown')] += 1
                    print(f"✅ 加载US法律数据: {file_path.name}")
                except Exception as e:
                    print(f"⚠️  加载失败 {file_path}: {e}")
