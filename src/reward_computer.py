...
"""
    ...
    def compute_reward(
        ...
        elif problem_type == "legal":
            # 法律任务: 使用法律奖励计算器
            reward = self._compute_legal_reward(problem, prediction, ground_truth, metadata)
        else:
            # 通用任务
    ...
    def _compute_legal_reward(
        ...
        """
        法律任务奖励计算 - 调用src/legal/reward.py的计算器
        ...
        # 从metadata获取jurisdiction和task_type
        jurisdiction = metadata.get('jurisdiction', 'CN') if metadata else 'CN'
        task_type = metadata.get('task_type', 'consultation') if metadata else 'consultation'

        # 尝试使用法律奖励计算器（同步版本）
        try:
            from src.legal.reward import LegalRewardComputer
            legal_computer = LegalRewardComputer(llm=None)  # 不使用LLM Judge

            # 简化评估（同步版本）
            # 1. 法律依据准确性
            legal_basis_score = self._evaluate_legal_basis_simple(pred_str, ground_truth, jurisdiction)

            # 2. 结论正确性
            conclusion_score = self._evaluate_legal_conclusion_simple(pred_str, ground_truth, task_type)

            # 3. 完整性检查
            completeness_score = self._evaluate_legal_completeness_simple(pred_str, task_type, jurisdiction)

            # 加权计算
            total_score = (
                0.35 * legal_basis_score +
                0.25 * 0.5 +  # 推理质量默认中等
            ...
            # 回退到通用评估
            return self._compute_general_reward(prediction, ground_truth)

    def _evaluate_legal_basis_simple(self, prediction: str, ground_truth: Any, jurisdiction: str) -> float:
        """简单法律依据评估"""
        # 提取法律引用
        if jurisdiction == "CN":
            pattern = r'《[^》]+》[第]?\d+[条款]?'
        else:
            pattern = r'\d+\s*U\.S\.C\.\s*§\s*\d+|[A-Z][a-z]+\s+v\.\s+[A-Z][a-z]+'

        import re
        citations = re.findall(pattern, prediction)
        ...
        if isinstance(ground_truth, dict):
            gt_articles = ground_truth.get('articles', []) or ground_truth.get('applicable_statutes', [])

        if not gt_articles:
            return 0.5 if citations else 0.0

        # 计算匹配度
        match_count = sum(1 for c in citations if any(str(a) in c or c in str(a) for a in gt_articles))
        return min(1.0, match_count / len(gt_articles) + 0.3) if match_count > 0 else 0.2

    def _evaluate_legal_conclusion_simple(self, prediction: str, ground_truth: Any, task_type: str) -> float:
        """简单结论正确性评估"""
        if isinstance(ground_truth, dict):
            # 案件预测：检查罪名
            if task_type == 'case_prediction':
                charges = ground_truth.get('charges', [])
                if charges and any(c in prediction for c in charges):
                    return 1.0
            ...
            if gt_answer:
                ...
                if gt_lower in pred_lower:
                    return 1.0
                # F1分数
                f1 = self._compute_f1_score(pred_lower, gt_lower)
                return f1

        return 0.5  # 无法评估时给中等分

    def _evaluate_legal_completeness_simple(self, prediction: str, task_type: str, jurisdiction: str) -> float:
        """简单完整性评估"""
        checklist = {
            'case_prediction': {
                'CN': ['罪名', '法条', '量刑'],
                'US': ['charge', 'statute', 'holding']
            },
            'statute_qa': {
                'CN': ['法条', '解释'],
                'US': ['statute', 'interpretation']
            },
            'consultation': {
                'CN': ['建议', '依据'],
                'US': ['advice', 'authority']
            ...
            'document_gen': {
                'CN': ['原告', '被告', '请求'],
                'US': ['plaintiff', 'defendant', 'relief']
            }
        }

        items = checklist.get(task_type, {}).get(jurisdiction, [])
        if not items:
            return 0.5

        found = sum(1 for item in items if item.lower() in prediction.lower())
