...
class RLWorkflowGenerator:
    ...
    def _build_generation_prompt(self, problem: str, problem_type: str, jurisdiction: str = "CN") -> str:
        """构建提示词，明确算子 API，让模型自主学习选择"""

        # 法律任务专用模板
        if problem_type == "legal":
            return self._build_legal_prompt(problem, jurisdiction)
...
"""
    ...
    def _build_legal_prompt(self, problem: str, jurisdiction: str = "CN") -> str:
        """构建法律任务的提示词模板"""

        if jurisdiction == "CN":
            system_context = "中国法律体系（大陆法系，成文法为主）"
            operators_desc = """
1. DirectAnswer(llm) - 直接生成法律答案
   Call: await self.direct_answer(question=str, context=str, jurisdiction="CN")
   Returns: {{'answer': str, 'confidence': float, 'jurisdiction': str}}

2. CaseLearning(llm, retriever) - 案例学习（检索指导性案例）
   Call: await self.case_learning(question=str, jurisdiction="CN", top_k=3)
   Returns: {{'learning_record': str, 'retrieved_cases': list, 'key_insights': list}}

3. StatuteLearning(llm, retriever) - 法条学习（检索法律条文+司法解释）
   Call: await self.statute_learning(question=str, jurisdiction="CN", legal_domain=str)
   Returns: {{'learning_record': str, 'retrieved_statutes': list, 'applicable_rules': list}}

4. Debate(llm) - 辩论投票（原告/被告/法官视角）
   Call: await self.debate(question=str, context=str, jurisdiction="CN", rounds=2)
   Returns: {{'final_answer': str, 'debate_log': list, 'vote_result': dict}}

5. LegalEnsemble(llm) - 集成选择（并行生成多个答案选最优）
   Call: await self.legal_ensemble(question=str, context=str, jurisdiction="CN", num_candidates=3)
   Returns: {{'final_answer': str, 'all_candidates': list, 'scores': list}}

6. LegalRevise(llm) - 法律修订（检查法条引用格式、罪名表述）
   Call: await self.legal_revise(answer=str, question=str, jurisdiction="CN")
   Returns: {{'revised_answer': str, 'revision_log': str, 'issues_found': list}}
"""
            example_workflow = """
    # 典型案件分析流程:
    # 1. 案例学习
    case_result = await self.case_learning(question=problem, jurisdiction="CN", top_k=3)

    # 2. 法条学习
    statute_result = await self.statute_learning(question=problem, jurisdiction="CN")

    # 3. 整合上下文
    context = f"{case_result['learning_record']}\\n{statute_result['learning_record']}"

    # 4. 辩论投票（可选，用于复杂案件）
    debate_result = await self.debate(question=problem, context=context, jurisdiction="CN")

    # 5. 修订检查
    final_result = await self.legal_revise(answer=debate_result['final_answer'], question=problem)

    return final_result['revised_answer'], self.llm.get_usage_summary()["total_cost"]
"""
        else:  # US
            system_context = "U.S. Legal System (Common Law, Case Precedent Based)"
            operators_desc = """
1. DirectAnswer(llm) - Generate direct legal answer
   Call: await self.direct_answer(question=str, context=str, jurisdiction="US")
   Returns: {{'answer': str, 'confidence': float, 'jurisdiction': str}}

2. CaseLearning(llm, retriever) - Case learning (retrieve binding/persuasive precedents)
   Call: await self.case_learning(question=str, jurisdiction="US", top_k=3)
   Returns: {{'learning_record': str, 'retrieved_cases': list, 'key_insights': list, 'precedent_type': str}}

3. StatuteLearning(llm, retriever) - Statute learning (U.S. Code + CFR)
   Call: await self.statute_learning(question=str, jurisdiction="US", legal_domain=str)
   Returns: {{'learning_record': str, 'retrieved_statutes': list, 'applicable_rules': list}}

4. Debate(llm) - Debate voting (Plaintiff/Defendant/Judge perspectives)
   Call: await self.debate(question=str, context=str, jurisdiction="US", rounds=2)
   Returns: {{'final_answer': str, 'debate_log': list, 'vote_result': dict}}

5. LegalEnsemble(llm) - Ensemble selection (parallel generation, select best)
   Call: await self.legal_ensemble(question=str, context=str, jurisdiction="US", num_candidates=3)
   Returns: {{'final_answer': str, 'all_candidates': list, 'scores': list}}

6. LegalRevise(llm) - Legal revision (check Bluebook citations, legal terminology)
   Call: await self.legal_revise(answer=str, question=str, jurisdiction="US")
   Returns: {{'revised_answer': str, 'revision_log': str, 'issues_found': list}}
"""
            example_workflow = """
    # Typical case analysis workflow:
    # 1. Case Learning
    case_result = await self.case_learning(question=problem, jurisdiction="US", top_k=3)

    # 2. Statute Learning
    statute_result = await self.statute_learning(question=problem, jurisdiction="US")

    # 3. Combine context
    context = f"{case_result['learning_record']}\\n{statute_result['learning_record']}"

    # 4. Debate voting (optional, for complex cases)
    debate_result = await self.debate(question=problem, context=context, jurisdiction="US")
...
"""
        ...
        prompt = f"""Generate a Python Workflow class to solve the LEGAL problem.

Legal System Context: {system_context}

CRITICAL RULES for LEGAL problems:
- Design your workflow to leverage legal operators for case analysis, statute lookup, and reasoning
- Always specify jurisdiction="{jurisdiction}" in operator calls
- For complex cases, use CaseLearning + StatuteLearning + Debate
- For simple queries, use StatuteLearning + DirectAnswer
- Always consider using LegalRevise for final answer polish

Available Legal Operators:
{operators_desc}

Template:

import workspace.legal.workflows.template.operator as operator
from src.legal.operators import DirectAnswer, CaseLearning, StatuteLearning, Debate, LegalEnsemble, LegalRevise
from src.legal.retriever import LegalRetriever
from scripts.async_llm import create_llm_instance
from scripts.evaluator import DatasetType
...
class Workflow:
    def __init__(self, name: str, llm_config, dataset: DatasetType):
        ...
        self.retriever.initialize(['{jurisdiction}'])

        # Initialize legal operators
        self.direct_answer = DirectAnswer(self.llm)
        ...
        self.legal_ensemble = LegalEnsemble(self.llm)
...
Generate your complete Workflow class:
"""
        return prompt
